...
//     // Find all expenses where groupId is undefined and the only participant is the current user
...
//       )
//       .filter((q) => {
//         // Filter to ensure the splits array contains exactly one item
//         // and that item belongs to the current user
//         return (
//           q.and(
//             q.eq(q.field("splits").length, 1),
//             q.eq(q.field("splits")[0].userId, user._id)
//           )
//         );
//       })
//       .collect();

//     // Sum the total amount of all personal expenses
...
export const getPersonalExpenses = query({
  ...
      // Step 1: Query for expenses paid by the user with no groupId
      const candidateExpenses = await ctx.db
        .query("expenses")
        .withIndex("by_user_and_group", (q) =>
          q.eq("paidByUserId", user._id).eq("groupId", undefined)
        )
  ...
      // Step 2: Use a client-side filter to find expenses with a single split
      const personalExpenses = candidateExpenses.filter(expense => {
        // Check if the splits array has exactly one item
        if (expense.splits.length !== 1) {
          return false;
        }
        // Check if that single split belongs to the current user
        return expense.splits[0].userId === user._id;
      });
